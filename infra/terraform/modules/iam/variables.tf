...
variable "oidc_provider_url" { type = string  default = "" } # without the https:// scheme
variable "media_bucket_arns" { type = list(string) }
variable "dynamodb_table_arn"{ type = string }
variable "secret_arns"       { type = list(string) default = [] }
variable "enable_github_oidc" { type = bool   default = false }
variable "github_repo"        { type = string default = "your-org/streamflix" }
